...
#   scripts/release.sh v0.1.4 --prep-changelog # print a grouped CHANGELOG.md draft for v<prev>..HEAD, then exit
#
# Required env:
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --prep-changelog) PREP_CHANGELOG=1 ;;
    -h|--help) usage ;;
    *) fail "unknown arg: $1" ;;
  esac
  shift
...
# ---------- changelog draft mode ----------
# Prints a grouped draft for the vX.Y.Z section (range: previous tag..HEAD),
# then exits. The workflow (see RELEASE.md): draft -> curate by hand ->
# commit CHANGELOG.md to main -> run the release. Preparing the section on
# main (not the release branch) means both remotes receive it through the
# normal push flow with no merge-back. No clean-tree requirement: the user
# is about to edit CHANGELOG.md.
if [[ "$PREP_CHANGELOG" -eq 1 ]]; then
  CURRENT_BRANCH="$(git rev-parse --abbrev-ref HEAD)"
  if [[ "$CURRENT_BRANCH" != "main" ]]; then
    fail "changelog drafts must be prepared on main (currently on $CURRENT_BRANCH)"
  fi
  PREV_TAG="$(git tag --sort=-version:refname | grep -v "^$TAG\$" | head -1 || true)"
  RANGE="${PREV_TAG:+$PREV_TAG..}HEAD"
  log "drafting changelog section for $TAG (range: $RANGE)"
  git log --no-merges --pretty=format:'%s' "$RANGE" | python3 -c "
import sys, datetime
groups = {'feat': 'Features', 'fix': 'Fixes', 'docs': 'Docs & tests',
          'test': 'Docs & tests', 'perf': 'Internal', 'refactor': 'Internal',
          'style': 'Internal'}
out = {}
for line in sys.stdin:
    ...
    if head == 'chore':
        continue  # release chores never appear in the curated log
    out.setdefault(groups.get(head, 'Internal'), []).append(line)
print('## [$TAG] — %s' % datetime.date.today().isoformat())
print()
for g in ('Features', 'Fixes', 'Docs & tests', 'Internal'):
    if out.get(g):
        print('### ' + g)
        for s in out[g]:
            print('- ' + s)
        print()
"
  log "DRAFT above - curate it (merge related commits into user-facing bullets), paste under the header of CHANGELOG.md, commit to main, then run this script without --prep-changelog"
  exit 0
fi
...
# Changelog section check (warning, not fatal - mirrors the Cargo sync
# check above; the GitHub Release body prefers the curated section).
if [[ -f CHANGELOG.md ]] && ! grep -q "^## \[$TAG\]" CHANGELOG.md; then
  log "WARNING: CHANGELOG.md has no '## [$TAG]' section - draft one with 'scripts/release.sh $TAG --prep-changelog', curate, and commit to main before releasing"
fi
...
if [[ "$GH_RELEASE" -eq 0 ]]; then
  ...
  # Prefer the curated CHANGELOG.md section for this tag; fall back to the
  # flat commit list (pre-v0.1.3 behavior).
  CHANGELOG_SECTION="$(awk -v t="## [$TAG]" 'index($0, t)==1 {f=1; next} /^## \[/ && f {f=0} f' CHANGELOG.md 2>/dev/null || true)"
