...
RESPONSE 侧（修复上游响应）
  ...
  └─ 流形状修复（stream-shape heal）  ←  当前无，未来 merge_fragmented 在此列
```
...
- 默认两字段全 ON（`Default for HealGates` 显式实现，注释解释：deny-by-default
  会静默关掉所有修复，与设计意图相反）。
...
| 方向 | response |
| 子类 | 内容修复 |
| 适用路径 | **chat + responses**（两条路径都有同形态泄漏） |
| 触发条件 | 文本里出现 DSML marker：`<｜DSML｜…>`（V3.2/V4 Pro）或 `<｜｜DSML｜｜…>`（V4 Flash，**双横杠优先**因为更具体） |
| 修复做法 | 把泄漏 markup 抠出来变成结构化 tool call，并从可见文本里剥掉 |
| 状态位 | `HealGates::dsml: bool`，默认 `true` |
| Chat 流式 hook | `src/convert/response.rs::StreamConverter::on_chunk`（每 chunk 走 `dsml_filter.push(delta.content)`） |
| Chat 非流式 hook | `src/proxy/chat.rs:478` 调 `heal_dsml_chat_message(&mut choice.message)` |
| Responses 流式 hook | `src/heal/responses.rs::ResponsesStreamHealer::push_event`（`response.output_text.delta` → `delta` 字段过 DSML 管道） |
| Responses 非流式 hook | `src/proxy/passthrough.rs:413` 调 `heal_responses_body(&body, heal)` |

DSML 解析细节：
- 模块：`src/heal/dsml.rs`（677 行）+ `dsml_tests.rs`（386 行）。
- 两条 dialect：`SINGLE_BAR` 与 `DOUBLE_BAR`（常量 `DsmlDialect`），
  探测顺序**双横杠优先**（NOTES 里有注释解释：`<｜` 不是 `<｜｜DSML｜｜`
  的子串，但探测更具体的形态在分隔符将来变化时更独立）。
- 解析策略：参数按**形态**读，不是按 tag 名读 —— V4 Flash 的 self-closing
  参数没有 body，其值只能在 attribute 里（`string="…"`）。
- 阻断策略：malformed invoke 用 balanced-end 恢复，外层 dialect 没平衡
  就放弃内层合法的 call（防误执行嵌套调用）。
- 调用 ID：upstream 漏 ID 时统一替换为合成的 `call_dsml_<uuid>`（与
  Chat 路径的 `call_<uuid>` 形状不同，仅历史命名差，语义一致）。
...
| 子类 | 内容修复 |
| 适用路径 | **chat + responses** |
| 触发条件 | 文本含 think marker：`<thinking>` / `<think>` / `◁think▷`（Kimi 变体）+ 对应闭合 |
...
观察者+改写者姿态）的对照 —— 字段名故意保持相似，以便看出对应关系：

| 字段 | StreamConverter（chat） | ResponsesStreamHealer（responses） |
|---|---|---|
| 持有 filter | `dsml_filter`, `think_filter` | `dsml`, `think` |
| 文本累积 | `acc.text`, `acc.reasoning` | `healed_text`, `reasoning_text` |
| 消息 item id | `msg_item_id: String`（**turn 初构造，整 turn 复用**） | `message_item_id: Option<String>`（**第一个 message item.added 时记下**） |
| 推理 item id | `reasoning_item_id: String`（**turn 初构造，整 turn 复用**） | `reasoning: Option<(id, idx)>`（lazy 创建） |
| 消息 output_index | `message_output_index: Option<usize>`（lazy 一次性） | `message_output_index: Option<usize>` |
| 下一个分配 index | `next_output_index: usize`（从 0 起） | `next_index: usize`（从 `INJECT_INDEX_BASE = 10_000` 起，避开真实 index） |
| tool call 累积 | `tool_calls: BTreeMap<usize, (call_id, name, args)>` | `injected_calls: Vec<(fc_id, call_id, name, args, idx)>` |
| finish 幂等 | `finish_emitted: bool` | `finished: bool` |
| 「天然不碎片」机制 | `message_output_index: Option<usize>` 整 turn 至多 None→Some 一次；重复 delta 复用同一 idx | 无 —— 依赖上游合规流（**这是 merge_fragmented 想修的边界**） |

`HealGates` 在两个 state machine 里的接线方式：
...
- `HealGates` 抽象（虽然只装 dsml + think 两字段，但所有 chat/responses
  的 content heal 都从这过）。
...
- **stream-shape heal 仅 responses 有意义** —— chat 是「按规范构造」，
  `message_output_index: Option<usize>` 整个 turn 至多 None→Some 一次
  是不变量；responses 是「按上游转发」，依赖上游合规流。
...
- review #9：`heal_responses_body` 的 `fired` 只在 reasoning 非空时设，
  空 think 块 (`<think></think>`) 被剥但 `fired` 仍 false，函数会
...
未来在 `HealGates` 里加第三字段时（如 `merge_fragmented`，即
MiniMax M3 Responses 网关把每 chunk 发成独立 item 的修复），参见：
- 设计 spec：`docs/superpowers/specs/<date>-<topic>-design.md`（待写）
- 本档案到时更新：§3 总览表加行、§6 加 6.5 节、§7 模块目录加 `merge.rs`、
  §8 state machine 对照表加 FragmentedItemMerger 列、§12 总结从
  「stream-shape heal 仅 responses」改为「stream-shape heal 在 responses 上
  由 merge_fragmented 提供」。
