...
│   ├── config.rs           # TOML types + validation + hot reload (1,262)
...
│   ├── proxy/
│   │   ├── mod.rs             # axum routing + dispatch + client-version observation (846)
...
│   ├── upstream.rs         # SSE parser + key resolution (parse_sse_block + parse_preserved_event) (668)
│   ├── catalog.rs          # gen-catalog + build_catalog_value + bundled-model hide-override discovery (1,219)
│   ├── models_cache.rs     # CatalogCache: route-fingerprint + template-mtime invalidation for live /models; and, when `hide_bundled_models` is on, re-probes `codex debug models --bundled` on the same 60s cadence (761)
